        ...
        <div v-else class="user-item-container">
            ...
            <div v-for="user in userList" :key="user.user_id" class="user-item">
                <div v-if="user.photo_id">
                    <img :src="user.profilePicData" alt="User Profile Picture" class="profile-picture">
                </div>
                <div v-else>
                    <img src="../assets/blank_profile_pic.webp" alt="Default Profile Picture" class="profile-picture">
                </div>
                <a @click="visitUserProfile(user.id, user.username, user.email)" class="user-name">{{ user.username }}
                </a>
...
export default {
    ...
    methods: {
        ...
        async fetchPostLikes(postId) {
            try {
                const response = await axios.get(this.$store.state.API + `/getPostLikes/${postId}`);
        ...
        async fetchCommentLikes(commentId) {
            try {
                const response = await axios.get(this.$store.state.API + `/getCommentLikes/${commentId}`);
                // Add the username to each user in the response
                this.likeList.push(response.data);
                this.likeList = this.likeList.flat();
        ...
        async fetchUsers(likeList) {
            try {
                for (var entry in likeList) {
                    const response = await axios.get(this.$store.state.API + `/users/${likeList[entry].user_id}`);
