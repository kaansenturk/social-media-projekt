<template>
    <div class="row">
        ...
        <div class="col-md-0" id="comment-item">
            ...
            <button @click="likeComment(comment.id, this.$store.state.logged_user_id)" class="btn">
...
export default {
    ...
    async mounted() {
        ...
        this.likedComments[this.comment.id] = await this.isCommentLiked(this.comment.id, this.$store.state.logged_user_id);
    ...
    methods: {
        ...
        async fetchData() {
            try {
                const response = await axios.get(this.API + `/users/${this.comment.user_id}`);
