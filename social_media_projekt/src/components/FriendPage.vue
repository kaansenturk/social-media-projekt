...
export default {
  ...
  async mounted() {
    ...
    const response1 = axios.get(
      this.$store.state.API + `/readFollowees/${userId}`
    );
    this.followeeNumber = (await response1).data;
    const response2 = axios.get(
      this.$store.state.API + `/readFollowers/${userId}`
    );
    ...
    async fetchUserData(userId) {
      try {
        const response = await axios.get(
          this.$store.state.API + `/users/${userId}`
        );
    ...
    async unfollowUser() {
      try {
        const response = await axios.post(
          this.$store.state.API + "/unfollowUser",
          null,
          {
            params: {
